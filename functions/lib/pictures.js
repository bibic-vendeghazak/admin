...
module.exports.generateThumbnail = (object, context) => {
  ...
  return bucket.file(filePath).download({
    ...
    return Promise.all(sizes.map((size, index) => sharp(tempFilePath).resize(size).toFile(tempThumbFilePaths[index])));
  }).then(() => {
    ...
    return Promise.all(thumbFilePaths.map((thumbFilePath, index) => bucket.upload(tempThumbFilePaths[index], {
    ...
    return Promise.all([bucket.file(filePath).getSignedUrl(config), ...thumbFileNames.map(thumbFileName => bucket.file(path.join(dirName, thumbFileName)).getSignedUrl(config))]);
  }).then(([original, ...rest]) => {
    ...
    return admin.database().ref(`${dirName}/pictures`).push(pictures);
  });
};
...
module.exports.deletePicture = (snapshot, baseURL) => {
  ...
  return Promise.all([bucket.file(`${baseURL}/${fileName}`).delete(), ...sizes.map(size => bucket.file(`${baseURL}/thumb_${size}_${fileName}`).delete())]).catch(console.error);
};